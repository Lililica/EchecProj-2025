...
struct PieceView {
  ...
  std::optional<glm::vec3> previousPos;
};
...
public:
  ...
  std::pair<int, int> get_pos_selected_piece() const {
    return _selectedPiece.has_value() ? _selectedPiece.value().pos
                                      : std::make_pair(-1, -1);
  };
  std::vector<std::pair<int, int>> get_possible_pos() const {
    return _selectedPiece.has_value() ? _selectedPiece.value().case_possible
                                      : std::vector<std::pair<int, int>>{};
  };
  std::vector<std::pair<int, int>> get_attack_possible() const {
    return _selectedPiece.has_value() ? _selectedPiece.value().attack_possible
                                      : std::vector<std::pair<int, int>>{};
  };
  PieceView *get_first_view() { return &_pieceView; };
